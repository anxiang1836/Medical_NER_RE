...
def f1_score4ner(pre_docs: List[Document], source_docs: List[Document], style='all'):
    ...
    for pre_doc, source_doc in zip(pre_docs, source_docs):
        ...
        pre_enti = [(ent.name,ent.category,ent.start_pos,ent.end_pos) for ent in pre_entities]
        source_enti = [(ent.name,ent.category,ent.start_pos,ent.end_pos) for ent in source_entities]
        
        entities_pair.append((pre_enti,source_enti))
